...
- The text language refuses a name bound twice, a `let` of `t`, `x` or `out`, a socket wired twice, a knob set twice and a group, `panel` or `requires` inside a group, and `flyback-cli check --json` reports it by line.
