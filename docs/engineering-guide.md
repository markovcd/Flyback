...
`Toolbar`, `PresetSlot`. `MainWindow` builds them by hand in its constructor,
lays them out, and keeps its layout, its keys, full screen and the closing
question as `partial` files. There are no view models, and that has been decided
twice.
