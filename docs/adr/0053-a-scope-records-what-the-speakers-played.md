...
## Amendment, 2026-09-10: two seconds, written down twice
...
grew to 10^1.5 seconds (about 31.62 s) for the Probe's sake. A Scope or Meter
asked for anything past two seconds got two seconds anyway, stretched across
the full width and labelled as if it were the window that was actually asked
for. A Probe and a Scope on the same patch, both set to the same window, then
drew visibly different amounts of history for a signal with no memory in it at
all — not the disagreement this record calls out as the useful kind, just a
buffer too small for the control in front of it.

The part worth recording is that the number was in **two** places, and they
were not the same kind of thing. One sized the ring (`DelayState.TraceSamples`).
The other clamped what a window was allowed to compile to at all
(`PatchCompiler.WindowOf`), so the refill never asked for more than two seconds
no matter how much ring there was. They agreed with each other, which is why
neither looked wrong on its own and why the first attempt at this fixed only
the ring and changed nothing anybody could see. A cap enforced twice needs
both raised or neither; raising the storage while something upstream still
refuses to ask for it is indistinguishable from having done nothing.

So the ceiling is one constant now — `DelayState.MaxWindowSeconds` — and both
the clamp and the ring size read it. Thirty-two seconds, rounding the knob's
actual top up to something round, so the knob's range sits strictly inside the
ring rather than exactly against it.

The fix is otherwise the same shape as the original decision: the ring stays a
fixed length, sized once, rather than reallocated as the knob turns — that
reasoning has not changed, only the number it is fixed at.

**Two megabytes per Scope** becomes about twenty-three. Sixteen times the
memory for a sixteen-times-longer past, at the same rate the original figure
was struck — nobody thought two seconds was cheap and thirty-one-and-a-bit is
not free either. It remains allocated only when the patch has a Scope or Meter
in it, and not otherwise.

The tests had pinned the bottom of the window's travel and not the top, which
