# ADR-0007: Values are register slots; scalars broadcast to colors

**Status:** Accepted · 2026-08-11

## Context

Two kinds of value travel down wires: a scalar signal, and a color. The
register machine ([0005](0005-compile-to-a-flat-register-machine.md)) stores
only `float`s, so colors need a representation, and the maths modules need to
...
A color occupies three *consecutive* registers. `Component(i)` is where
...
The interpreter never learns that colors exist. It executes scalar ops on a
...
color conversion at the end pays for three-wide arithmetic only at the end,
which is the common shape.

Multiplying a color by a scalar works with no special case, and that is what
makes `Any` ports viable ([0010](0010-any-typed-ports-for-polymorphic-maths.md)):
one `Multiply` module handles `color * scalar`, `scalar * scalar` and
`color * color` because `Binary` resolves widths itself.

Contiguity is load-bearing. `color.split` returns `Slot.Scalar(base)`,
...
color must keep its three registers adjacent; `Emitter.ToColor` and
`Emitter.Combine` are the only places that do, and both allocate a block of three.

Narrowing a color to a scalar is lossy and needs a rule. `ToScalar` uses
