...
/// </summary>
/// <remarks>
/// A cell's index is its position among the ops of its kind, and a position is not
/// an identity: add one oscillator upstream and every accumulator after it belongs
/// to a different module, so a renderer with only the counts to go on can either
/// throw all of it away or hand an oscillator a stranger's phase. The compiler
/// already knows whose op it is emitting, and writing that down turns "the shape
/// changed" into "these three cells are new and the rest carried on".
/// <para>
/// One node may own several cells of a kind — a supersaw is seven accumulators —
/// so a cell is matched to the one in the same position within its owner.
/// </para>
/// </remarks>
    ...
    IReadOnlyList<Guid> Units)
{
    ...
    /// and the two of those are the same two in every program that has them — see
    /// <see cref="Emitter.Interval"/> and <see cref="Emitter.HasMemory"/>.
    /// </summary>
    /// <remarks>
    /// A fixed name rather than the first module that happened to ask: attributed to
    /// its first asker, the cell would be dropped the moment that module was
    /// deleted, and the interval it measures would read as a jump.
    /// </remarks>
    public static Guid Shared { get; } = new("00000000-0000-0000-0000-0000000f1B00");

    /// <summary>
    /// Where each of <paramref name="mine"/> should read its previous value from, or
    /// -1 for a cell with no history to take. <see cref="Guid.Empty"/> matches
    /// nothing, including itself: not knowing who owns a cell is not the same as
    /// knowing two cells are the same cell.
    /// </summary>
