...
- **A DI container for the editor and the viewer.** Microsoft.Extensions.DependencyInjection, as the preset server already has through ASP.NET: `MainWindow`'s twenty constructor arguments, the regions' constructors and statics such as `Startup.Plugins` resolved from one composition root. Needs an ADR, since ADR-0148 wires by hand.
