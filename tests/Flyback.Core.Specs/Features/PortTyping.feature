Feature: Scalars and colours travel down the same wires
  A patch mixes single signals and three-channel colours freely. Widths are
  resolved when the patch compiles, so the interpreter never learns that colours
  exist and a scalar-only patch never pays for three-wide arithmetic.

  Specified by ADR-0007 and ADR-0010.

  Scenario: A scalar entering a colour port broadcasts to all three channels
  ...
  Scenario: A colour entering a scalar port narrows to its luma
    Given a patch containing:
      | name   | module       |
      | red    | colour.rgb   |
      | turn   | space.rotate |
    ...
    And "red" output "colour" is wired to "turn" input "x"
    And "turn" output "x" is wired to "screen" input "colour"
    When the patch is compiled
    Then the centre pixel is about 0.2126, 0.2126, 0.2126

  Scenario: One Multiply handles a colour scaled by a scalar
    Given a patch containing:
      | name   | module     |
      | tint   | colour.rgb |
      | half   | math.mul   |
    ...
    And "tint" output "colour" is wired to "half" input "a"
    And "half" input "b" is set to 0.5
    And "half" output "out" is wired to "screen" input "colour"
    When the patch is compiled
    Then compilation reports no issues
    And the centre pixel is about 0.5, 0.25, 0

  Scenario: Splitting a colour recovers its channels
    Given a patch containing:
      | name    | module       |
      | tint    | colour.rgb   |
    ...
    And "tint" output "colour" is wired to "channel" input "colour"
    And "channel" output "g" is wired to "screen" input "colour"
    When the patch is compiled
