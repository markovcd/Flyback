...
**How to apply:** a shell or Rider started before the variable was set does not see it; `release-key.sh` and the site read the user environment directly for that reason. A new key makes the Figures or Fractals an editor installed from the local site a different plugin, so remove it from the editor and install it again after one is made. To try a release, run `./release.sh` rather than dispatching the workflow: it is the same build, into `dist/`, published nowhere. Run it only in the main worktree with `main` checked out (the `build-artifacts` skill).
