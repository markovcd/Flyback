...
If it is missing, make a new one without asking: `. ./release-key.sh` does it, and so do a build of the app and a Release run of the site. The app's public key is never edited by hand: a local build derives it from the variable into `obj/`, and `make.sh` and `release.sh` hand it to Docker, while the committed `release-key.pem` stays the real one. Never ask for the real release key and never put it in the variable.
