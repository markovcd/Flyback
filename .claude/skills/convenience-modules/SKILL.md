...
- **The shipped assistant briefing must stay under four fifths of `AssistantSettings.DefaultProseBudget`** (80,000 of 100,000); it is about 56,000. Write a new module's description and each socket's `Help` for the agent as much as the panel: the description is what the module is for and how its sockets work together, the help what one socket is for, and both count against the budget. A socket named in `SocketHelp` (x, y, freq, phase, amp, bias, mix, …) needs no help unless it means something else here. `ProsePolicyTests.What_ships_is_described_in_full_under_the_default_budget` is what fails.
