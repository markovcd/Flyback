...
- **The shipped assistant briefing must stay under four fifths of `AssistantSettings.DefaultProseBudget`** (80,000 of 100,000). Only descriptions count against it: the briefing says what each module is for, and the agent reads each socket's `Help` through `describe_module`. Write the description as what the module is for and how its sockets work together, and give every socket its help, the words from `SocketHelp` (`SocketHelp.Freq`, `SocketHelp.Position`, …) where it is a standard one. `ProsePolicyTests.What_ships_is_described_in_full_under_the_default_budget` is what fails.
